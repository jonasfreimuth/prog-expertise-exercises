...
int main(int argc, char const *argv[]) {
    ...
    vector< string > valid_commands = { "getN", "getSequence", "grepSeq" };

    argparse::ArgumentParser parser("jf-FASTA-parser.bin");
    ...
    try {  
        ...
        for (auto id = ids.begin(); id != ids.end(); id ++) {
            ...
            if (find(commands.begin(), commands.end(), "getN") != commands.end()) {
                ...
                if (!regex_match( * id, regex("[a-zA-Z0-9_|]+"))) {
                        cout << "ID " << * id << " is not valid!\n";
                        exit(1);
                }
            } else if (find(commands.begin(), commands.end(), "getSeq") != commands.end()) {
                // TODO: Put in appropirate pattern
                if (!regex_match( * id, regex("[a-zA-Z0-9_|]+"))) {
                        cout << "ID " << * id << " is not valid!\n";
                        exit(1);
                }
            } else if (find(commands.begin(), commands.end(), "grepSeq") != commands.end()) {
                // TODO: Put in appropirate pattern
                if (!regex_match( * id, regex("[ABCDEFGHIKLMNPQRSTVWXYZ]+"))) {
                        cout << "ID " << * id << " is not valid!\n";
                        exit(1);
                }
            }
    ...
    for (auto cmd = commands.begin(); cmd != commands.end(); cmd ++) {
        ...
        if (* cmd == "getN") {
            for (auto file = files.begin(); file != files.end(); file ++) {
                ...
                tmp_res = f_parser.getN( * file);                    
                res.insert(res.end(), tmp_res.begin(), tmp_res.end());

            }
        } else if (* cmd == "getSequence") {
            for (auto file = files.begin(); file != files.end(); file ++) {
                
                for (auto id = ids.begin(); id != ids.end(); id ++) {
                    tmp_res = f_parser.getSeq( * file, * id);                    
                    res.insert(res.end(), tmp_res.begin(), tmp_res.end());
                }   
